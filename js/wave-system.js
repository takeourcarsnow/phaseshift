...
if (typeof WavePoint === 'undefined') {
    class WavePoint {
        ...
        grad(hash, x, y) {
            // If the hash is not a number (i.e. undefined), return 0 to avoid errors.
            if (typeof hash !== 'number') {
                console.warn("grad: hash is undefined, returning 0");
                return 0;
            }

            // Ensure the permutation array is valid.
            if (!this.p || !Array.isArray(this.p) || this.p.length < 512) {
                this.p = new Array(512);
                for (let i = 0; i < 256; i++) {
                    this.p[i] = this.p[i + 256] = Math.floor(Math.random() * 256);
                }
